...
4. 依赖安装：可选 `npm install`——本地化 PI（`@automatalabs/pi-acp`）与 `pi-mcp-adapter`（optionalDependencies，离线/安装失败不阻断）；未安装时 PI 适配器经 npx 拉起（首跑会下载、较慢），OpenCode/OMP 不受影响，网关核心仍零依赖。
